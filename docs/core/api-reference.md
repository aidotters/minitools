...
class MediumCollector:
    ...
    async def fetch_article_content(
        ...
        """
        記事のコンテンツをJina AI Readerで取得（`--use-jina` 指定時のみ呼び出される）

        デフォルト動作ではメールプレビューテキストを使用し、本メソッドはスキップされる。
        `scripts/medium.py` 側で `--use-jina` フラグの有無により分岐する。
        Mediumはサイト側でJina経由のアクセスをブロックするため、本メソッドは
        `scrapers/jina_reader.py` の `JinaReader` ではなく `MediumCollector` 内で
        独自実装（User-Agentローテーション等）を持つ。
...
`https://r.jina.ai/{url}` 経由で任意のWebページの全文Markdownを取得する非同期クライアント。
`google-alerts-translate` で英語ニュース・ブログ記事の本文取得に使用される。

**ファイル:** `minitools/scrapers/jina_reader.py`

**注意:** Mediumには対応しない。Mediumはサイト側でJina AI Readerをブロックしており、`MediumCollector` がサイト固有の回避策を実装している。
...
class JinaReader:
    ...
    async def fetch_markdown(
        ...
        Returns:
            Markdown 文字列。Cloudflareブロック検出時や失敗時は空文字列。
        """

    @staticmethod
    def extract_metadata(markdown: str) -> dict[str, Optional[str]]:
        """
        Jina 出力の Markdown ヘッダから ``Title:`` と ``Published Time:`` を抽出する

        Args:
            markdown: ``fetch_markdown()`` の戻り値

        Returns:
            ``{"title": str | None, "published_at": "YYYY-MM-DD" | None}``
            ``Published Time:`` は ISO 8601 から ``YYYY-MM-DD`` への整形を試み、
            失敗時は ``None``。
        """
```
...
```python
reader = JinaReader()
markdown = await reader.fetch_markdown("https://example.com/article")
if markdown:
    metadata = JinaReader.extract_metadata(markdown)
    print(metadata["title"], metadata["published_at"])
```
...
> 補助 dataclass（`TrendSummary`、`KeywordSummary`、`TimelineSummary`、`ProcessResult`）は [Dataclass定義](#dataclass定義) を参照。
...
`NotionPublisher.find_page_by_url()` の返り値を格納する NamedTuple。
...
class PageInfo(NamedTuple):
    """find_page_by_url の返り値"""
    page_id: str
    is_translated: bool
```

`is_translated` は Notion ページの `Translated` チェックボックスの値で、既に翻訳済みのページに対する重複処理（再翻訳）をスキップするために使用される。

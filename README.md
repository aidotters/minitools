...
特定の送信元メール内に含まれる YouTube 動画を要約し、Slack 配信 + Notion 保存する。字幕を優先取得し（無ければ Whisper にフォールバック）、日本語の要約文＋ポイント箇条書きを生成する。`settings.yaml` の `youtube_mail_digest.profiles` に「送信元 → 保存先」を複数定義して実行する。重複動画はプロファイルごとに `processed.json` に記録してスキップする。

```bash
# 全プロファイルを直近24時間分処理
uv run youtube-mail-digest

# 取得期間を指定 / 特定日のメールを処理
uv run youtube-mail-digest --hours 24
uv run youtube-mail-digest --date 2026-06-07

# 特定プロファイルのみ処理
uv run youtube-mail-digest --profile ai-newsletter

# Slackのみ（Notion保存なし） / Notionのみ（Slack配信なし）
uv run youtube-mail-digest --profile x --no-notion
uv run youtube-mail-digest --profile x --no-slack

# 各プロファイル先頭1動画のみ（テスト） / Slack・Notion送信なしのプレビュー
uv run youtube-mail-digest --test
uv run youtube-mail-digest --dry-run

# 要約 LLM プロバイダを指定（gemini/openai/ollama）
uv run youtube-mail-digest --provider openai
```
...
### YouTube Mail Digest ツール

特定の送信元メールに含まれる YouTube 動画を要約し、Slack 配信 + Notion 保存します。プロファイル単位で「送信元 → 保存先」を `settings.yaml` に定義して運用します。

**特徴**:
- 特定送信元（`from:`）のメール本文から YouTube 動画 URL を抽出（watch / youtu.be / shorts / embed を正規化し video_id で重複排除）
- 字幕を優先取得（手動→自動、ja→en）、字幕が無ければ MLX Whisper にフォールバック
- 日本語の要約文＋ポイント箇条書き（3〜7個）を生成（デフォルト Gemini）
- プロファイルごとに Slack / Notion の保存先を個別指定
- 重複動画はプロファイルごとに `processed.json` に記録してスキップ

**必要な環境**:
- Gmail API 認証（`credentials.json` / `token.pickle`、`google-alerts` / `medium` と共有）
- `settings.yaml` の `youtube_mail_digest.profiles`（送信元・保存先）定義
- Slack Webhook（例: `SLACK_YOUTUBE_MAIL_DIGEST_WEBHOOK_URL`）／ Notion API キー
- 字幕の無い動画で Whisper フォールバックを使う場合は `uv sync --extra whisper`

**オプション**:
- `--profile`: 処理するプロファイル名（未指定で全プロファイル）
- `--hours`: 過去何時間分のメールを取得するか（デフォルト: 24）
- `--date`: 特定日のメールを取得（YYYY-MM-DD）
- `--no-slack`: Slack 配信を無効化
- `--no-notion`: Notion 保存を無効化
- `--test`: 各プロファイル先頭1動画のみ処理
- `--dry-run`: Slack/Notion 送信なしのプレビュー
- `--provider`: 要約 LLM プロバイダ（gemini/openai/ollama）
- `--debug`: デバッグログ

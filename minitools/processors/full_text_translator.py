...
7. **文体統一（最重要）**: 文体ベースは「である調（常体）」で固定し、ドキュメント全体・チャンク全体で一貫させてください。途中から敬体に切り替えてはいけません。
   - 採用する語尾の例: 「〜である」「〜だ」「〜する」「〜した」「〜なる」「〜と考えられる」「〜と言える」「〜が分かる」
   - 禁止する語尾の例: 「〜です」「〜ます」「〜でしょう」「〜ください」「〜しましょう」「〜いたします」
   - 段落内・文書内で文体を混在させないでください。1 文だけ敬体に揺れることも避けてください。
   - 見出し、箇条書き、図表のキャプション、注釈、引用内の地の文も常体で統一してください。
   - 例外: 原文の引用ブロック（`>`）内に第三者の発話として敬体の引用が含まれる場合は、その引用に限り原文の語調を保持して構いません。

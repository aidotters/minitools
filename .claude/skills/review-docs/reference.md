# review-docs — 詳細リファレンス

> review-docs の各ステップ（1〜8）の詳細手順・出力フォーマット、および再レビューモード（--from-report: F1〜F5）の手順集。
> SKILL.md の「レビューの流れ」から参照されます。
...
### 2.3 CLIコマンドカバレッジ（README 使用方法）

`pyproject.toml [project.scripts]` の全コマンドが **README.md の使用方法セクションに記載されているか**を確認する。README の使用方法（CLIコマンド）は手動管理範囲ではなく**自動更新範囲**であり、新規コマンドの未記載は完全性の欠落として扱う。

```
# pyproject.toml の全コマンドを取得
Read('pyproject.toml')  → [project.scripts] の全コマンド名

# README の使用方法に記載されているか確認
for コマンド in 全コマンド:
    Grep('<コマンド>', 'README.md')
→ 未記載コマンド = 全コマンド − README記載コマンド
```

未記載コマンドは「README は手動管理だから任意」と判断**しない**。完全性レポートの「未記載CLIコマンド」テーブルに記録し、改善提案では**優先度「高」**に分類する（理由は 2.4 を参照）。
...
| **CLIコマンドカバレッジ（README）** | **100%** | **pyproject.toml `[project.scripts]` + README 使用方法の文書比較** |

**CLIコマンド未記載を優先度「高」とする理由:** README はユーザー向け公式ドキュメントであり、使用方法は自動更新範囲。かつ feature-flow の `--with-docs` ループは優先度「高」のみを自動修正する。中/低だと申し送りされるだけで自動修正されず、新規コマンドが README に載らないまま完了してしまう。確実に閉じるため「高」とし、update-docs の `--from-report`（未記載CLIコマンドの追加手順）で修正させる。
...
### 未記載CLIコマンド
| コマンド | pyproject.toml | README 使用方法 | 推奨対応 |
|----------|----------------|-----------------|----------|
| `xxx-command` | 定義あり | 未記載 | README 使用方法に使用例を追加（優先度: 高） |
...
> **必須ルール（取りこぼし防止）:** 「1. 完全性 → 未記載CLIコマンド」テーブルに1件以上ある場合、改善提案 **優先度「高」に必ず1項目立てる**（例: `- [ ] README に <コマンド> の使用例が未記載: update-docs --from-report で使用方法に追加`）。完全性テーブルに記録しただけで改善提案 高に挙げないと、feature-flow の `--with-docs` ループは優先度「高」の件数だけを数えるため 高=0 と誤判定し、README が修正されないまま合格してしまう。

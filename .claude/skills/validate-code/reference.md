# validate-code — 詳細リファレンス

> validate-code の各ステップの詳細手順・コマンド例・出力フォーマット、および再検証モード（--from-report）の手順集。
> SKILL.md の「実行ステップ（概要）」から参照されます。
...
```bash
Bash('mypy [対象パス] --show-error-codes --no-error-summary')
```
...
```bash
# ruff check
Bash('ruff check {対象ファイル1} {対象ファイル2} ... --output-format=grouped')

# mypy
Bash('mypy {対象ファイル1} {対象ファイル2} ... --show-error-codes --no-error-summary')
```
...
```bash
Bash('pytest {関連テストファイル} -v')
```

関連テストの特定:
- steeringのtasklist.mdに記載されたテストファイル
- 変更されたモジュールに対応する`tests/test_*.py`

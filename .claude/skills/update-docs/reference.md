# update-docs — 詳細リファレンス

> update-docs の3モード（通常: ステップ1〜5 / Steeringフォルダ: S1〜S5 / レビュー結果修正: R1〜R4）の詳細手順・出力フォーマット・モード別の注意事項。
> SKILL.md の「モード別処理フロー」から参照されます。
...
**新規CLIコマンドは必ず README の使用方法に追加する（最重要）:**

README の「使用方法（CLIコマンド）」は**自動更新範囲**であり、手動管理範囲（概要・セットアップ手順・Docker関連）ではない。新規CLIコマンドを追加したのに README に書かないのは、宣言済みの更新責務の放棄である。「README は手動管理だから」を理由に使用方法セクションの更新をスキップしてはならない。

```
# pyproject.toml の全コマンドと README の使用方法を突き合わせる
Read('pyproject.toml')  → [project.scripts] の全コマンド名
Grep('<コマンド名>', 'README.md')
→ README 使用方法に出現しないコマンドは「使用例セクションを必ず追加」
```

**チェックポイント:**
- [ ] pyproject.toml `[project.scripts]` の**全コマンド**が README 使用方法に記載されている（新規コマンドの取りこぼしゼロ）
- [ ] 「使用方法」が実際のCLIコマンド（引数・オプション）と一致している
...
- `README.md`:
  ...
  - 概要、セットアップ手順は手動管理
  - Docker関連セクションは手動管理
  - **重要:** 手動管理は上記（概要・セットアップ・Docker）に**限る**。「README は手動管理範囲だから」を理由に**使用方法（CLIコマンド）の更新をスキップしてはならない**。新規CLIコマンドの使用方法追加は自動更新範囲であり、必須作業である
...
| `README.md` | **新規CLIコマンドがある場合は使用方法セクションを必ず更新**（自動更新範囲）。その他、該当セクション（プロジェクト構成・出力形式・設定・環境変数）の追加・更新がある場合も更新 |

**README の取りこぼし防止（Steeringフォルダモードでも必須）:**

steeringフォルダの実装で `pyproject.toml [project.scripts]` に新規コマンドが追加された場合、README 使用方法への追記は**スキップ不可**。実装範囲に新規CLIコマンドが含まれるかを必ず確認する:
```
Read('pyproject.toml')  → [project.scripts]
# design.md / tasklist.md で新規追加されたコマンドを特定
for 新規コマンド in 実装範囲の新規CLIコマンド:
    Grep('<新規コマンド>', 'README.md')  → 未記載なら使用方法セクションに必ず追加
```
「README は手動管理だから任意」と判断してはならない。手動管理範囲は概要・セットアップ手順・Docker関連のみ。
...
#### 未記載CLIコマンドの追加（README 使用方法）

レビューレポートで「README にCLIコマンドが未記載」と指摘された場合、README の使用方法セクションに使用例を追加して指摘を**閉じる**。これにより feature-flow の `--with-docs` ループ（優先度「高」を自動修正）が README の漏れを実際に解消できる。

```
# レビューレポートから未記載CLIコマンドを取得
未記載コマンド = "1. 完全性" → "未記載CLIコマンド" テーブル（または改善提案の該当項目）から抽出

# 各コマンドについて
for コマンド in 未記載コマンド:
    # 正確な引数・オプションを実装から取得
    Read('pyproject.toml')                 # [project.scripts] でエントリ確認
    Read('scripts/{コマンド}.py' など)       # argparse 定義から引数・オプションを把握

    # README の使用方法セクションに、既存コマンドと同じ記法で使用例を追加
    Read('README.md')
    Edit('README.md')  # 「使用方法」内の適切な位置に追加
```
